...
public:
	...
	static std::wstring ConvertToWString(const std::runtime_error& ex);
...
public:
	...
	// Constructor(s) and Destructor
	// -------------------------
	Audio(const tstring& filename, bool readBeats = false);
	...
	// Aubio
	AubioData GetCurrentDataSample() const;
	AubioData GetDataByIndex(int index) const;
...
private:	
	...
	std::vector<AubioData> m_AubioSampleData;

	// -------------------------
	// General Member Functions
	// -------------------------		
	void Create(const tstring& filename, bool readBeats = false);
