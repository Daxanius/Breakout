...
#include <aubio/aubio.h> // Audio stuff
#include <locale>
#include <codecvt>
#include <regex>
...
void GameEngine::RegisterLua(sol::state& m_Lua) {
	m_Lua.new_usertype<GameEngine>("GameEngine", sol::no_constructor,
															   "fillRect", [&](GameEngine* self, float left, float top, float right, float bottom) {
																   self->FillRect(static_cast<int>(roundf(left)), static_cast<int>(roundf(top)), static_cast<int>(roundf(right)), static_cast<int>(roundf(bottom)));
															   },
															   
															   "drawRect", [&](GameEngine* self, float left, float top, float right, float bottom) {
																  	self->DrawRect(static_cast<int>(roundf(left)), static_cast<int>(roundf(top)), static_cast<int>(roundf(right)), static_cast<int>(roundf(bottom)));
															   },
															   
															   "drawLine", [&](GameEngine* self, float x1, float y1, float x2, float y2) {
															   	self->DrawLine(static_cast<int>(roundf(x1)), static_cast<int>(roundf(y1)), static_cast<int>(roundf(x2)), static_cast<int>(roundf(y2)));
															   },

																 "drawOval", [&](GameEngine* self, float left, float top, float right, float bottom) {
																 	self->DrawOval(static_cast<int>(roundf(left)), static_cast<int>(roundf(top)), static_cast<int>(roundf(right)), static_cast<int>(roundf(bottom)));
																 },
																 
																 "fillOval", [&](GameEngine* self, float left, float top, float right, float bottom) {
																 	self->FillOval(static_cast<int>(roundf(left)), static_cast<int>(roundf(top)), static_cast<int>(roundf(right)), static_cast<int>(roundf(bottom)));
																 },

															  "messageBox", sol::overload(
																  [&](GameEngine* self, const std::wstring& message) {
																 	 self->MessageBox(message);
																  }
															  ),
															  
															  "messageContinue", sol::overload(
																  [&](GameEngine* self, const std::wstring& message) {
																 	 self->MessageContinue(message);
																  }
															  ),
															  
															  "setColor",
															  [&](GameEngine* self, int r, int g, int b) {
																  return self->SetColor(RGB(r, g, b));
															  },
															  ...
															  "getFrameRate",
															  [&](GameEngine* self) {
																  return (float)self->GetFrameRate();
															  },
															  
															  "isKeyDown",
															  [&](GameEngine* self, char key) {
																  return self->IsKeyDown(key);
															  },
															  
															  "drawString", sol::overload(
																  [&](GameEngine* self, const std::wstring& text, float left, float top) {
																 	 return self->DrawString(text, static_cast<int>(std::roundf(left)), static_cast<int>(std::roundf(top)));
																  },
																  [&](GameEngine* self, const std::wstring& text, float left, float top, float right, float bottom) {
																 	 return self->DrawString(text, static_cast<int>(std::roundf(left)), static_cast<int>(std::roundf(top)), static_cast<int>(std::roundf(right)), static_cast<int>(std::roundf(bottom)));
																  }
															  ),
															  ...
															  "showMouse", &GameEngine::ShowMousePointer,
															  "goFullScreen", &GameEngine::GoFullscreen,
															  "goWindowed", &GameEngine::GoWindowedMode
...
std::wstring GameEngine::ConvertToWString(const std::runtime_error& ex) {
	std::string errorMsg = ex.what();
	return std::wstring(errorMsg.begin(), errorMsg.end());
}

COLORREF GameEngine::FromRGB(int r, int g, int b) {
	return RGB(r, g, b);
}
...
bool GameEngine::FillOval(int left, int top, int right, int bottom) const {
	if (m_IsPainting) {
		...
		Gdiplus::Graphics graphics(m_HdcDraw);

		// Enable anti-aliasing
		graphics.SetSmoothingMode(Gdiplus::SmoothingMode::SmoothingModeAntiAlias);

		// Create a GDI+ brush with the current draw color
		Gdiplus::Color color(
			GetRValue(m_ColDraw),
			GetGValue(m_ColDraw),
			GetBValue(m_ColDraw)
		...
		// Draw the filled oval using GDI+
...
int GameEngine::GetFontSize() const {
	if (!m_FontDraw) {
	...
	HGDIOBJ oldFont = SelectObject(hdc, m_FontDraw);
...
void Audio::RegisterLua(sol::state& m_Lua) {
	m_Lua.new_usertype<AubioData>("AubioData",
																"lastMs", &AubioData::lastMs,
																"lastS", &AubioData::lastS,
																"last", &AubioData::last,
																"bpm", &AubioData::bpm,
																"confidence", &AubioData::confidence,
																"wasTatum", &AubioData::wasTatum
	);

	m_Lua.new_usertype<Audio>("Audio", sol::constructors<Audio(const tstring& filename, bool readBeats)>(),
														"play", &Audio::Play,
														...
														"getDataByIndex", &Audio::GetDataByIndex,
														"getPlaybackPosition", &Audio::GetPlaybackPosition,
														"getDataSize", &Audio::GetDataSize,
														"getCurrentDataSample", &Audio::GetCurrentDataSample
	);
}

Audio::Audio(const tstring& filename, bool readBeats)
{	
	tstring soundPath{ tstring(L"game/" + filename)};

	{	// separate block => the file stream will close 
		tifstream testExists(soundPath);
		if (!testExists.good()) throw FileNotFoundException{ soundPath };
	}

	size_t len{ soundPath.length() };
	if (len < 5) throw BadFilenameException{ soundPath };

	tstring suffix{ soundPath.substr(len - 4) };
	if (suffix == _T(".mp3") || suffix == _T(".wav") || suffix == _T(".mid"))
	{
		...
		m_Alias = buffer.str();
		m_Filename = soundPath;

		Create(soundPath, readBeats);
	}
	else throw UnsupportedFormatException{ soundPath };
}

void Audio::Create(const tstring& filename, bool readBeats)
{
	size_t len{ filename.length() };
	...
	// If we don't want to interpret the audio
	if (!readBeats) {
		return;
	}
	...
	filename_utf8 = std::regex_replace(filename_utf8, std::regex("mp3"), "wav"); // Aubio only works with wave files, make sure you have a variant available

	// Load in the audio and get the beat from it using aubio
	uint_t samplerate{ 44100 }; // Hardcoded little sample rate.
	uint_t win_size{ 2048 };		// Increase for better temporal resolution
	uint_t hop_size{ win_size / 4 };
	uint_t n_frames{}, read{};

	aubio_source_t* aubioSource{ new_aubio_source(filename_utf8.c_str(), samplerate, hop_size) };

	if (aubioSource == NULL) {
		return;
	}

	// Audio data to load in
	fvec_t* aubioSamples{ new_fvec(hop_size) };
	fvec_t* aubioTempoSamples{ new_fvec(hop_size) };

	// Read the tempo
	aubio_tempo_t* aubioTempo{ new_aubio_tempo("default", win_size, hop_size, samplerate) };
	// aubio_tempo_set_threshold(aubioTempo, 1.f);

	// Read in the audio information
	if (aubioSamples != NULL && aubioTempoSamples != NULL && aubioTempo != NULL) {
		do {
			aubio_source_do(aubioSource, aubioSamples, &read);
			aubio_tempo_do(aubioTempo, aubioSamples, aubioTempoSamples);

			m_AubioSampleData.emplace_back(
				aubio_tempo_get_last_ms(aubioTempo),
				aubio_tempo_get_last_s(aubioTempo),
				aubio_tempo_get_last(aubioTempo),
				aubio_tempo_get_bpm(aubioTempo),
				aubio_tempo_get_confidence(aubioTempo),
				aubio_tempo_was_tatum(aubioTempo)
			);
	...
	// Delete all aubio stuff
	if (aubioTempo != NULL) {
		del_aubio_tempo(aubioTempo);
		aubioTempo = nullptr;
	}

	if (aubioTempoSamples != NULL) {
		del_fvec(aubioTempoSamples);
		aubioTempoSamples = nullptr;
	}

	if (aubioSamples != NULL) {
		del_fvec(aubioSamples);
		aubioSamples = nullptr;
	}
...
Audio::~Audio()
{
	tstring sendString = _T("stop ") + m_Alias; // <-- Stop(); Doesn't call stop it just addeds it to the queue. Which never gets called
	mciSendString(sendString.c_str(), nullptr, 0, m_hWnd);

	sendString = tstring(_T("close ")) + m_Alias; // <-- Added space 
...
int Audio::GetPlaybackPosition() const {
	if (!m_Playing) {
		return 0; // If not playing, return 0 or some other indication of no playback.
	}

	TCHAR response[100] = { 0 };
	tstringstream buffer;

	// Query the current position in milliseconds
	buffer << _T("status ") + m_Alias + _T(" position");
	int result = mciSendString(buffer.str().c_str(), response, 100, NULL);
	...
	int position = 0;
	tstringstream responseStream(response);
	responseStream >> position;

	return position; // Return the playback position in milliseconds
}
...
AubioData Audio::GetCurrentDataSample() const {
	const int playBackMs{ GetPlaybackPosition() };

	for (const AubioData& data : m_AubioSampleData) {
		if (data.lastMs >= playBackMs) {
			return data;
		}
	}

	// Return if nothing found otherwise
	return {};
}

AubioData Audio::GetDataByIndex(int index) const {
	return m_AubioSampleData.at(index % m_AubioSampleData.size());
}

int Audio::GetDataSize() const {
	return m_AubioSampleData.size();
}
...
void Font::RegisterLua(sol::state& m_Lua) {
	m_Lua.new_usertype<Font>("Font", sol::constructors<Font(const tstring& fontName, bool bold, bool italic, bool underline, int size)>());
}
